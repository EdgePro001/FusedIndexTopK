...
// against per-row sampled thresholds and emitted as compact candidates instead
// of materializing the dense [Q, N] logits matrix.
...
namespace index_topk_perflab::fused_r13a {

using namespace deep_gemm;
using namespace deep_gemm::sm90;

__device__ __forceinline__ uint64_t itk_fused_r13a_pack_score_id(
...
#ifndef ITK_R13_KERNEL_NAME
#define ITK_R13_KERNEL_NAME itk_sm90_fp8_mqa_candidate_producer_r13a
#endif

template <bool STATIC_SEGMENT_COUNTERS, bool MASKED_REPAIR,
          bool PIPELINED_WGMMA,
...
__global__ __launch_bounds__(
    kNumTMAThreads + kNumMathThreads, kNumMathThreads == 128 ? 2 : 1)
void ITK_R13_KERNEL_NAME(
                         ...
                         const uint8_t* repair_flags,
                         const __grid_constant__ cute::TmaDescriptor tensor_map_q,
                         const __grid_constant__ cute::TmaDescriptor tensor_map_kv,
                         const __grid_constant__ cute::TmaDescriptor tensor_map_kv_scales,
                         const __grid_constant__ cute::TmaDescriptor tensor_map_weights) {
    // One persistent CTA owns two queries for their complete KV scan. It emits
    // compact per-row candidates and never materializes [BLOCK_Q, seq_len_kv].
    ...
    DG_STATIC_ASSERT(
        BLOCK_Q == 2 and
        ((BLOCK_KV == 128 and kNumMathThreads == 256) or
         (BLOCK_KV == 64 and kNumMathThreads == 128)),
        "fused R13 producer requires [2,128]/256 or [2,64]/128");
    DG_STATIC_ASSERT(TOP_K == 2048, "fused R13a requires exact Top-2048");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 16384, "fused R13a repair capacity mismatch");
    else
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 14080, "fused R13a candidate capacity mismatch");
    if constexpr (PIPELINED_WGMMA) {
        DG_STATIC_ASSERT(not STATIC_SEGMENT_COUNTERS,
                         "R15a preserves R13a dynamic segment counters");
        DG_STATIC_ASSERT(not MASKED_REPAIR,
                         "R15a changes only the fast candidate producer");
        DG_STATIC_ASSERT(BLOCK_KV == 128 and kNumMathThreads == 256,
                         "R15a requires the R13a producer shape");
    }
    DG_STATIC_ASSERT(kNumTMAThreads == 128,
                     "Invalid fused R13 specialized threads");
    DG_STATIC_ASSERT(
        (kNumMathWarps == 8 and kWarpSegments == 2) or
        (kNumMathWarps == 4 and kWarpSegments == 4),
        "R13 requires 16 logical segments across its math warps");
    DG_STATIC_ASSERT(CANDIDATE_CAPACITY % kNumCandidateSegments == 0,
                     "candidate segments must divide evenly");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(kSegmentCandidateCapacity == 1024,
                         "fused R13a repair segment mismatch");
    else
        DG_STATIC_ASSERT(kSegmentCandidateCapacity == 880,
                         "fused R13a segment capacity mismatch");

    // The repair launch has one CTA per query pair. A pair that passed the
    // first reducer returns before allocating pipeline work; if either row
    // failed, both rows share one WGMMA scan but only failed rows are emitted.
    if constexpr (MASKED_REPAIR) {
        if (blockIdx.x >= num_q_blocks)
            return;
        const uint32_t q_idx = blockIdx.x * BLOCK_Q;
        if (__ldg(repair_flags + q_idx) == 0 and
            __ldg(repair_flags + q_idx + 1) == 0)
            return;
    }
    ...
    // R15a keeps two 64-register WGMMA accumulator sets live. Hopper's
    // SETMAXNREG redistribution lets the math warp-groups borrow registers
    // released by the specialized TMA warp-group without changing CTA count.
    constexpr uint32_t kNumMathRegisters = PIPELINED_WGMMA ? 176 : 112;
    ...
    if (threadIdx.x >= kNumMathThreads) {
        // TMA warp-group for loading data. The remaining three specialized
        // warps are intentionally idle: candidate emission now happens directly
        // in the 16 math warps and requires no score-tile handoff.
        ...
        while (block_q_idx < num_q_blocks) {
            ...
            uint32_t q_indices[BLOCK_Q], row_start[BLOCK_Q], row_end[BLOCK_Q];
            float thresholds[BLOCK_Q];
            bool repair_rows[BLOCK_Q];
            int warp_candidate_counts[BLOCK_Q][kWarpSegments] = {{0}};
            int warp_candidate_counts_even[BLOCK_Q] = {0};
            int warp_candidate_counts_odd[BLOCK_Q] = {0};
            #pragma unroll
            for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                ...
                if constexpr (MASKED_REPAIR) {
                    ...
                    thresholds[i] = 0.0f;
                    repair_rows[i] = __ldg(repair_flags + q_idx) != 0;
                } else {
                    row_start[i] = 0;
            ...
            // Issue one WGMMA tile into the selected accumulator set. R13a
            // consumes it immediately; R15a may leave the group outstanding
            // while the previous accumulator set runs its CUDA-core epilogue.
            ...
            // Consume one completed WGMMA tile. This is bit-for-bit the R13a
            // reduction, threshold, ballot, and candidate emission path.
            const auto& consume_wgmma_tile = [&](const uint32_t& kv_block_idx,
                ...
                for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                    ...
                    bool active_0, active_1;
                    if constexpr (MASKED_REPAIR) {
                        active_0 = role == 0u and repair_rows[i] and
                            id_0 >= row_start[i] and id_0 < row_end[i];
                        active_1 = role == 1u and repair_rows[i] and
                            id_1 >= row_start[i] and id_1 < row_end[i];
                    } else {
                        active_0 = role == 0u and
                            id_0 < row_end[i] and v_0 >= thresholds[i];
                        active_1 = role == 1u and
                            id_1 < row_end[i] and v_1 >= thresholds[i];
                    }
                    ...
                    const uint32_t lower_lanes = lane_idx == 0
                        ? 0u : ((1u << lane_idx) - 1u);
                    const int base = STATIC_SEGMENT_COUNTERS
                        ? (segment_slot == 0u
                            ? warp_candidate_counts_even[i]
                            : warp_candidate_counts_odd[i])
                        : warp_candidate_counts[i][segment_slot];
                    const int position =
                        base + __popc(active_mask & lower_lanes);
                    const uint64_t segment_offset =
                        static_cast<uint64_t>(q_indices[i]) * CANDIDATE_CAPACITY +
                        segment_idx * kSegmentCandidateCapacity;
                    if (active_0 and position < kSegmentCandidateCapacity)
                        candidate_pairs[segment_offset + position] =
                            itk_fused_r13a_pack_score_id(v_0, id_0);
                    if (active_1 and position < kSegmentCandidateCapacity)
                        candidate_pairs[segment_offset + position] =
                            itk_fused_r13a_pack_score_id(v_1, id_1);
                    const int next_count = base + __popc(active_mask);
                    if constexpr (STATIC_SEGMENT_COUNTERS) {
                        if (segment_slot == 0u)
                            warp_candidate_counts_even[i] = next_count;
                        else
                            warp_candidate_counts_odd[i] = next_count;
                    } else {
            ...
            // Compute over KV blocks. The final wait<0> at the end of an R15a
            // row drains the outstanding group before Q and KV stages can be
            // recycled by the persistent scheduler.
            if constexpr (PIPELINED_WGMMA) {
                float next_accum[WGMMA::kNumAccum];
                float accum_scale_kv_0, accum_scale_kv_1;
                float next_scale_kv_0, next_scale_kv_1;
                ...
                for (uint32_t kv_block_idx = 0;
                     ...
                     kv_block_idx += 2u) {
                    if (kv_block_idx + 1u < num_kv_blocks) {
                        issue_wgmma_tile(
                            kv_block_idx + 1u,
                            next_accum,
                            next_scale_kv_0,
                            next_scale_kv_1);
                        // Retire the older group while keeping the newly
                        // issued tile eligible to run beside this epilogue.
                        warpgroup_wait<1>();
                    } else {
                        warpgroup_wait<0>();
                    }
                    ...
                    if (kv_block_idx + 1u < num_kv_blocks) {
                        if (kv_block_idx + 2u < num_kv_blocks) {
                            issue_wgmma_tile(
                                kv_block_idx + 2u,
                                accum,
                                accum_scale_kv_0,
                ...
                for (uint32_t kv_block_idx = 0;
                    ...
                    issue_wgmma_tile(
                        kv_block_idx, accum, scale_kv_0, scale_kv_1);
                    warpgroup_wait<0>();
                    consume_wgmma_tile(
                        kv_block_idx, accum, scale_kv_0, scale_kv_1);
                }
            ...
            // Publish each warp-owned segment length independently. The exact
            // reducer already scans all 16 lengths, so doing a second CTA-wide
            // reduction here only adds a barrier to the producer critical path.
            if (lane_idx == 0) {
                #pragma unroll
                for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                    if constexpr (STATIC_SEGMENT_COUNTERS) {
                        const uint64_t row_offset =
                            static_cast<uint64_t>(q_indices[i]) *
                            kNumCandidateSegments;
                        segment_counts[row_offset + warp_idx] =
                            warp_candidate_counts_even[i];
                        segment_counts[
                            row_offset + warp_idx + kNumMathWarps] =
                            warp_candidate_counts_odd[i];
                    } else {
                        #pragma unroll
