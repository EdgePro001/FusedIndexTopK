#!/usr/bin/env python3
"""Audit the R13a release fault-injection results."""
...
def _audit_case(name: str, result: dict[str, Any], expected: dict[str, Any]) -> None:
    if result.get("producer") != "r13a" or result.get("reducer") != "r11d":
        raise ValueError(f"{name} did not test the final producer/reducer pair")
    for key in (
        "failure_rows",
        "value_mismatch_rows",
...
def main() -> None:
    ...
    payload = {
        "schema_version": 1,
        "status": "passed",
        "method": "R13a/R11d path-specific fault injection and repeated exactness",
