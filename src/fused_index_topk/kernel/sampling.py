"""Random-token threshold sampling for FusedIndexTopK."""
...
_SOURCE = Path(__file__).resolve().parent / "csrc" / "sampling_threshold.cu"
_EXTENSION: Any | None = None


def sample_elements_for_context(context_tokens: int) -> int:
    """Use the measured 0.5% independent-token sampling schedule."""
...
def guarded_sample_rank(
    ...
    *,
    ...
    """Return the conservative descending sample rank."""
...
def load_sampling_extension(*, verbose: bool = False) -> Any:
    ...
    if CUDA_HOME is None:
        raise RuntimeError("CUDA_HOME is required to build FusedIndexTopK sampling")
    identity = path_fingerprint(_SOURCE)
    module_name = f"fused_index_topk_sampling_{identity['sha256'][:12]}"
    _EXTENSION = load(
        name=module_name,
        sources=[str(_SOURCE)],
