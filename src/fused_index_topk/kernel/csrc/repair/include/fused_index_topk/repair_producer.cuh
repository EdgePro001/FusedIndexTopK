...
namespace fused_index_topk::kernel::repair {
...
void itk_fused_index_topk_repair_producer(
    ...
    DG_STATIC_ASSERT(BLOCK_Q == 2 and BLOCK_KV == 256, "fused repair producer requires [2, 256] score tiles");
    DG_STATIC_ASSERT(TOP_K == 2048, "fused repair producer requires exact Top-2048");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 16384, "fused repair producer repair capacity mismatch");
    else
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 14080, "fused repair producer candidate capacity mismatch");
    DG_STATIC_ASSERT(kNumTMAThreads == 128 and kNumMathThreads == 512, "Invalid fused repair producer threads");
    DG_STATIC_ASSERT(CANDIDATE_CAPACITY % kNumMathWarps == 0, "candidate segments must divide evenly");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(kWarpCandidateCapacity == 1024, "fused repair producer repair segment mismatch");
    else
        DG_STATIC_ASSERT(kWarpCandidateCapacity == 880, "fused repair producer segment capacity mismatch");
