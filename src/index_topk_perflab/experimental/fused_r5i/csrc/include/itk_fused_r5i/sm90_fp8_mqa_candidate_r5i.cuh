...
__device__ __forceinline__ uint64_t itk_fused_r5i_pack_score_id(
...
void itk_sm90_fp8_mqa_candidate_producer_r5i(
    ...
    static constexpr uint32_t kWarpCandidateCapacity =
        CANDIDATE_CAPACITY / kNumMathWarps;
    // Prefetch TMA descriptors
    DG_STATIC_ASSERT(BLOCK_Q == 2 and BLOCK_KV == 256, "fused R5i requires [2, 256] score tiles");
    DG_STATIC_ASSERT(TOP_K == 2048, "fused R5i requires exact Top-2048");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 16384, "fused R5i repair capacity mismatch");
    else
        DG_STATIC_ASSERT(CANDIDATE_CAPACITY == 14080, "fused R5i candidate capacity mismatch");
    DG_STATIC_ASSERT(kNumTMAThreads == 128 and kNumMathThreads == 512, "Invalid fused R5i threads");
    DG_STATIC_ASSERT(CANDIDATE_CAPACITY % kNumMathWarps == 0, "candidate segments must divide evenly");
    if constexpr (MASKED_REPAIR)
        DG_STATIC_ASSERT(kWarpCandidateCapacity == 1024, "fused R5i repair segment mismatch");
    else
        DG_STATIC_ASSERT(kWarpCandidateCapacity == 880, "fused R5i segment capacity mismatch");
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (block_q_idx < num_q_blocks) {
            ...
            int warp_candidate_counts[BLOCK_Q] = {0};
            #pragma unroll
            for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
            ...
            for (uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; ++ kv_block_idx) {
                ...
                for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                    ...
                    const uint32_t lower_lanes = lane_idx == 0
                        ? 0u : ((1u << lane_idx) - 1u);
                    const int base = warp_candidate_counts[i];
                    const int position =
                        base + __popc(active_mask & lower_lanes);
                    const uint64_t segment_offset =
                        static_cast<uint64_t>(q_indices[i]) * CANDIDATE_CAPACITY +
                        warp_idx * kWarpCandidateCapacity;
                    if (active_0 and position < kWarpCandidateCapacity)
                        candidate_pairs[segment_offset + position] =
                            itk_fused_r5i_pack_score_id(v_0, id_0);
                    if (active_1 and position < kWarpCandidateCapacity)
                        candidate_pairs[segment_offset + position] =
                            itk_fused_r5i_pack_score_id(v_1, id_1);
                    warp_candidate_counts[i] = base + __popc(active_mask);
                }
            }
            ...
            if (lane_idx == 0) {
                #pragma unroll
                for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                    const int count = warp_candidate_counts[i];
                    segment_counts[
                        static_cast<uint64_t>(q_indices[i]) * kNumMathWarps +
                        warp_idx] = count;
                }
            }
            num_total_kv_blocks += num_kv_blocks;
