...
#ifndef ITK_LONG_MATH_SCHEDULE
#define ITK_LONG_MATH_SCHEDULE 2
#endif
#ifndef ITK_LONG_MATH_REGISTERS
#define ITK_LONG_MATH_REGISTERS 168
#endif
#ifndef ITK_LONG_CACHE_WEIGHTS
#define ITK_LONG_CACHE_WEIGHTS 0
#endif
#include "long_cta_topk.cuh"
...
#ifndef ITK_LONG_KERNEL_NAME
#define ITK_LONG_KERNEL_NAME itk_fused_index_topk_long
#endif
...
void ITK_LONG_KERNEL_NAME(
    ...
    static constexpr uint32_t kNumCandidateSegments = 8;
    ...
    DG_STATIC_ASSERT((CANDIDATE_CAPACITY == 7680 and kNumQStages == 3 and kCandidateSlots == 1 and not kOverlap) or
                     ...
                     "long-context fused TopK supports parallel control or compact double-slot layout");
    DG_STATIC_ASSERT(kNumTMAThreads == 128,
                     "Invalid long-context fused TopK specialized threads");
    DG_STATIC_ASSERT(
        kNumMathWarps == 8 and kWarpSegments == 1,
        "long-context fused TopK requires 8 logical segments across 8 math warps");
    ...
    constexpr uint32_t kNumMathRegisters = ITK_LONG_MATH_REGISTERS;
    DG_STATIC_ASSERT(ITK_LONG_MATH_SCHEDULE == 1 or ITK_LONG_MATH_SCHEDULE == 2,
                     "long-context fused TopK only uses the verified paired/steady WGMMA control flows");
    DG_STATIC_ASSERT(kNumMathRegisters >= 168 and kNumMathRegisters <= 216 and
                     kNumMathRegisters % 8 == 0, "Invalid long-context fused TopK Math register budget");
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        if (not is_tma_load_warp) {
            ...
            while (block_q_idx < num_q_blocks) {
                ...
                for (uint32_t i = 0; i < BLOCK_Q; ++i) {
                    ...
                    if constexpr (ITK_LONG_DIAGNOSTIC) row_trace = trace + 16 + i * 24;
        ...
        while (block_q_idx < num_q_blocks) {
            ...
            if constexpr (ITK_LONG_CACHE_WEIGHTS) {
            // Read weights
            #pragma unroll
            for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                #pragma unroll
            ...
            const auto& consume_wgmma_tile = [&](const uint32_t& kv_block_idx,
                ...
                for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                    ...
                    const auto& transform = [&](const uint32_t& j) {
                        ...
                        float weight;
                        if constexpr (not ITK_LONG_CACHE_WEIGHTS) {
                    ...
                    const uint32_t lower_lanes = lane_idx == 0
                        ? 0u : ((1u << lane_idx) - 1u);
                    int base;
                    if constexpr (ITK_LONG_TUNING & 4) {
                        ...
                        static_assert(kWarpSegments == 1);
                        base = warp_candidate_counts[i][0];
                    } else {
                    ...
                    if (active and position >= kSegmentCandidateCapacity and
                        position < kSegmentCandidateCapacity + kSpillPerSegment) {
                        const uint64_t row = block_q_idx * BLOCK_Q + i;
                        const uint64_t spill_offset = row * kSpillPerRow +
                            segment_idx * kSpillPerSegment + position - kSegmentCandidateCapacity;
                        spill_pairs[spill_offset] = active_0
                            ? pack_score_id(v_0, id_0)
                            : pack_score_id(v_1, id_1);
                    }
                    const int next_count = base + __popc(active_mask);
                    if constexpr (ITK_LONG_TUNING & 4) {
                        warp_candidate_counts[i][0] = next_count;
                    } else {
            ...
            if constexpr (ITK_LONG_MATH_SCHEDULE == 1) {
            ...
            } else if constexpr (ITK_LONG_MATH_SCHEDULE == 2) {
            ...
            } else if constexpr (ITK_LONG_MATH_SCHEDULE == 3) {
            ...
            // Each math thread publishes both SMEM and ordinary GMEM stores.
            // CUTLASS mbarrier arrive.release / wait.acquire orders all generic
