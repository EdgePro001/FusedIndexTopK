...
void itk_fused_r6f_segmented_candidate_radix(
  ...
  for (int row = blockIdx.x; row < rows; row += row_stride) {
    ...
    if constexpr (!kMaskedRepair) {
      int prefix_rank = 0;
      const uint32_t prefix_threshold = radix_select_prefix16_shared(
          scores,
          total_count,
          kTopK,
          histogram,
      ...
      for (int base = 0; base < total_count; base += kBlockThreads) {
        ...
        if (is_selected && selected_position < kWarpTailCapacity &&
            total_count + selected_position < kCandidateCapacity) {
      ...
      if (tx == 0) {
        ...
        if (greater_counter != expected_greater ||
            ...
            total_count + selected_counter > kCandidateCapacity) {
...
void configure() {
  ...
  TORCH_CHECK(status == cudaSuccess,
              "cannot opt in to segmented R6f candidate shared memory: ",
  ...
  TORCH_CHECK(status == cudaSuccess,
              "cannot opt in to masked R6f repair shared memory: ",
              cudaGetErrorString(status));
}
...
pybind11::dict resource_report() {
  ...
  cudaError_t status = cudaFuncGetAttributes(
      ...
      itk_fused_r6f_segmented_candidate_radix<
          kFastSegmentCapacity, false>);
  TORCH_CHECK(status == cudaSuccess,
              "cannot query segmented R6f candidate kernel: ",
              cudaGetErrorString(status));
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      kFastCandidateBytes);
  TORCH_CHECK(status == cudaSuccess,
              "cannot calculate segmented R6f candidate occupancy: ",
              cudaGetErrorString(status));
  ...
  status = cudaFuncGetAttributes(
      ...
      itk_fused_r6f_segmented_candidate_radix<
          kRepairSegmentCapacity, true>);
  TORCH_CHECK(status == cudaSuccess,
              "cannot query masked R6f repair kernel: ",
              cudaGetErrorString(status));
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      kRepairCandidateBytes);
  TORCH_CHECK(status == cudaSuccess,
              "cannot calculate masked R6f repair occupancy: ",
              cudaGetErrorString(status));
...
void topk_out_impl(torch::Tensor candidate_pairs,
  ...
  TORCH_CHECK(status == cudaSuccess,
              kMaskedRepair
                  ? "masked R6f repair radix launch failed: "
                  : "segmented R6f candidate radix launch failed: ",
              cudaGetErrorString(status));
}
