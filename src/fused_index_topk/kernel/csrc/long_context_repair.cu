...
__global__ void itk_finalize_repair(
...
int repair_grid_blocks(int rows, int device) {
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "cannot query multiprocessor count for long-context repair: ",
...
void local_topk_pairs_out(
  ...
  TORCH_CHECK(rows64 > 0 && rows64 <= INT32_MAX, "invalid repair row count");
  TORCH_CHECK(
      logical_offset >= 0 && logical_offset <= UINT32_MAX,
      "invalid repair logical offset");
  ...
  TORCH_CHECK(
      ...
      "all local reducer tensors must share one CUDA device");
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "chunk-local TopK launch failed: ",
      cudaGetErrorString(status));
}
...
void merge_topk_pairs_out(
  ...
  TORCH_CHECK(
      accumulator_counts.get_device() == device &&
          ...
          failure_flags.get_device() == device &&
          repair_error_flags.get_device() == device,
      "all merge tensors must share one CUDA device");
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "hierarchical pair merge launch failed: ",
      cudaGetErrorString(status));
}
...
void finalize_repair_out(
  ...
  TORCH_CHECK(
      accumulator_counts.get_device() == device &&
          ...
          failure_flags.get_device() == device &&
          repair_error_flags.get_device() == device,
      "all finalize tensors must share one CUDA device");
  c10::cuda::CUDAGuard guard(accumulator_pairs.device());
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream(device).stream();
  const int rows = static_cast<int>(rows64);
  itk_finalize_repair<<<
...
void configure() {
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "cannot opt in to chunk-local shared memory: ",
      cudaGetErrorString(status));
}
...
pybind11::dict resource_report() {
  ...
  TORCH_CHECK(
      cudaFuncGetAttributes(&local_attributes, itk_chunk_local_topk) ==
          cudaSuccess,
      "cannot query local reducer resources");
  TORCH_CHECK(
      cudaFuncGetAttributes(&merge_attributes, itk_merge_topk_pairs) ==
          cudaSuccess,
      "cannot query merge reducer resources");
...
__global__ void itk_update_chunk_ranges(
    const int* starts,
    const int* ends,
    int* chunk_starts,
    int* chunk_ends,
  ...
  if (i >= rows * chunks) return;
  const int chunk = i / rows;
  const int row = i % rows;
  const int offset = chunk * kCandidateCapacity;
  const int width = min(kCandidateCapacity, columns - offset);
  chunk_starts[i] = min(width, max(0, starts[row] - offset));
  chunk_ends[i] = min(width, max(0, ends[row] - offset));
}

void update_ranges_out(
    torch::Tensor start,
    torch::Tensor end,
    torch::Tensor chunk_start,
    torch::Tensor chunk_end,
    int columns) {
  const int rows = start.numel();
  const int chunks = (columns + kCandidateCapacity - 1) / kCandidateCapacity;
  TORCH_CHECK(columns >= 8192 && columns <= 163840 && rows > 0, "invalid repair ranges");
  check_vector(start, at::kInt, rows, "start");
  check_vector(end, at::kInt, rows, "end");
  check_matrix(chunk_start, at::kInt, chunks, rows, "chunk_start");
  check_matrix(chunk_end, at::kInt, chunks, rows, "chunk_end");
  TORCH_CHECK(
      start.device() == end.device() && start.device() == chunk_start.device() &&
          start.device() == chunk_end.device(),
      "repair ranges must share one CUDA device");
  c10::cuda::CUDAGuard guard(start.device());
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream(start.get_device()).stream();
  itk_update_chunk_ranges<<<(rows * chunks + 255) / 256, 256, 0, stream>>>(
      start.data_ptr<int>(),
      end.data_ptr<int>(),
      chunk_start.data_ptr<int>(),
      chunk_end.data_ptr<int>(),
      rows,
      columns,
      chunks);
  TORCH_CHECK(cudaGetLastError() == cudaSuccess, "repair range update launch failed");
}

}  // namespace

PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("update_ranges_out", &update_ranges_out, "Build per-chunk causal ranges");
  module.def("configure", &configure, "Configure long-context repair");
  module.def("resource_report", &resource_report, "Report repair resources");
  module.def(
