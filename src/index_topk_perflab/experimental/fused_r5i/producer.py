"""Build and load the producer-side DeepGEMM + R5i candidate fusion."""
...
_KERNEL_HEADER = (
    ...
    / "include"
    / "itk_fused_r5i"
    / "sm90_fp8_mqa_candidate_r5i.cuh"
)
...
def _require_sha(path: Path, expected: str) -> None:
    ...
    if actual != expected:
        raise RuntimeError(
            f"fused R5i upstream mismatch for {path}: "
...
def _deepgemm_source_root() -> Path:
    ...
    if not raw:
        raise RuntimeError(
            ...
            "the fused R5i experiment never modifies that checkout"
        )
...
def load_candidate_producer(deep_gemm: Any, *, verbose: bool = False) -> Any:
    ...
    if CUDA_HOME is None:
        raise RuntimeError("CUDA_HOME is required to build the fused R5i producer")
    ...
    if not _KERNEL_HEADER.is_file():
        raise RuntimeError(f"missing fused R5i device source: {_KERNEL_HEADER}")
    ...
    if missing:
        raise RuntimeError(
            f"missing fused R5i build include directories: {missing}"
        )

    module_name = f"itk_fused_r5i_producer_host_{source_sha[:12]}"
    _EXTENSION = load(
        name=module_name,
