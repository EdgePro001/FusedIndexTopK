...
constexpr int kExpectedProducerSmemBytes = 101508;
constexpr int kSmallTileProducerSmemBytes = 76148;

template <bool kMaskedRepair, bool kStaticSegmentCounters, bool kSmallKVTile,
          bool kPipelinedWgmma>
class SM90FP8MQACandidateR13aRuntime final
    : public LaunchRuntime<SM90FP8MQACandidateR13aRuntime<
          kMaskedRepair, kStaticSegmentCounters, kSmallKVTile,
          kPipelinedWgmma>> {
public:
    struct Args {
        int seq_len;
    ...
    static std::string generate_impl(const Args& args) {
        ...
        constexpr const char* kernel_name = kPipelinedWgmma
            ? "itk_sm90_fp8_mqa_candidate_producer_r15a"
            : (kSmallKVTile
                ? "itk_sm90_fp8_mqa_candidate_producer_r13d"
                : (kStaticSegmentCounters
                    ? "itk_sm90_fp8_mqa_candidate_producer_r13b"
                    : "itk_sm90_fp8_mqa_candidate_producer_r13a"));
        constexpr int block_kv = kSmallKVTile ? 64 : kBlockKV;
        constexpr int math_threads = kSmallKVTile ? 128 : kNumMathThreads;
        return fmt::format(R"(
#define ITK_R13_KERNEL_NAME {}
#include "{}"

using namespace index_topk_perflab::fused_r13a;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&ITK_R13_KERNEL_NAME<
        {}, {}, {}, 64, 128, 2, {}, 3, 3, 128, {}, 2048, {}
    >);
}}
)",
            kernel_name,
            custom_kernel_header,
            kStaticSegmentCounters ? "true" : "false",
            kMaskedRepair ? "true" : "false",
            kPipelinedWgmma ? "true" : "false",
            block_kv,
            math_threads,
            candidate_capacity);
    }
...
inline void init_jit(
    ...
    SM90FP8MQACandidateR13aRuntime<false, false, false, false>::configure(
        custom_kernel_header, custom_source_sha);
    SM90FP8MQACandidateR13aRuntime<true, false, false, false>::configure(
        custom_kernel_header, custom_source_sha);
    SM90FP8MQACandidateR13aRuntime<false, true, false, false>::configure(
        custom_kernel_header, custom_source_sha);
    SM90FP8MQACandidateR13aRuntime<false, false, true, false>::configure(
        custom_kernel_header, custom_source_sha);
    SM90FP8MQACandidateR13aRuntime<false, false, false, true>::configure(
        custom_kernel_header, custom_source_sha);
}

template <
    bool kMaskedRepair,
    bool kStaticSegmentCounters = false,
    bool kSmallKVTile = false,
    bool kPipelinedWgmma = false>
inline void fp8_mqa_candidate_impl(
    ...
    constexpr int block_kv = kSmallKVTile ? 64 : kBlockKV;
    constexpr int math_threads = kSmallKVTile ? 128 : kNumMathThreads;
    constexpr int expected_smem_bytes = kSmallKVTile
        ? kSmallTileProducerSmemBytes : kExpectedProducerSmemBytes;
    ...
    smem_size += kNumKVStages * block_kv * head_dim * kv.element_size();
    smem_size +=
        kNumQStages * kBlockQ * num_heads * weights.element_size();
    smem_size += kNumKVStages * block_kv * kv_scales.element_size();
    smem_size +=
        (kNumQStages * 2 + kNumKVStages * 2 + (math_threads / 128) * 2) * 8;
    smem_size += 4;
    DG_HOST_ASSERT(smem_size == expected_smem_bytes);
    DG_HOST_ASSERT(smem_size <= SM90ArchSpec::smem_capacity);

    using Runtime = SM90FP8MQACandidateR13aRuntime<
        kMaskedRepair, kStaticSegmentCounters, kSmallKVTile,
        kPipelinedWgmma>;
    const typename Runtime::Args args = {
        ...
        .launch_args = LaunchArgs(
            kMaskedRepair
                ...
                : device_runtime->get_num_sms() * 2,
            kNumSpecializedThreads + math_threads,
            smem_size),
    };
    ...
    const std::string build_name =
        std::string(kMaskedRepair
            ? "itk_sm90_fp8_mqa_masked_repair_producer_r13a_"
            : (kPipelinedWgmma
                ? "itk_sm90_fp8_mqa_candidate_producer_r15a_"
                : (kSmallKVTile
                    ? "itk_sm90_fp8_mqa_candidate_producer_r13d_"
                    : (kStaticSegmentCounters
                        ? "itk_sm90_fp8_mqa_candidate_producer_r13b_"
                        : "itk_sm90_fp8_mqa_candidate_producer_r13a_")))) +
        Runtime::custom_source_sha.substr(0, 12);
    const auto runtime = compiler->build(build_name, code);
    Runtime::launch(runtime, args);
}

inline void fp8_mqa_candidate_r13a_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
    ...
    const torch::Tensor& candidate_pairs,
    const torch::Tensor& segment_counts) {
    fp8_mqa_candidate_impl<false, false, false>(
        q,
        kv,
        kv_scales,
        weights,
...
inline void fp8_mqa_candidate_r13b_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
    ...
    const torch::Tensor& candidate_pairs,
    const torch::Tensor& segment_counts) {
    fp8_mqa_candidate_impl<false, true, false>(
        q,
        kv,
        kv_scales,
        weights,
...
inline void fp8_mqa_candidate_r13d_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
    ...
    const torch::Tensor& candidate_pairs,
    const torch::Tensor& segment_counts) {
    fp8_mqa_candidate_impl<false, false, true>(
        q,
        kv,
        kv_scales,
        weights,
...
inline void fp8_mqa_candidate_r15a_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
    ...
    const torch::Tensor& candidate_pairs,
    const torch::Tensor& segment_counts) {
    fp8_mqa_candidate_impl<false, false, false, true>(
        q,
        kv,
        kv_scales,
        weights,
...
inline void fp8_mqa_candidate_repair_r13a_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
    ...
    const torch::Tensor& candidate_pairs,
    const torch::Tensor& segment_counts) {
    fp8_mqa_candidate_impl<true, false, false>(
        q,
        kv,
        kv_scales,
        weights,
