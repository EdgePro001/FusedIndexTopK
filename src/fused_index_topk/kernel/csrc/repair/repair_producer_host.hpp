...
class RepairProducerRuntime final
    : public LaunchRuntime<RepairProducerRuntime<kMaskedRepair>> {
public:
    struct Args {
        int seq_len;
        int seq_len_kv;
...
using namespace fused_index_topk::kernel::repair;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&itk_fused_index_topk_repair_producer<
...
inline void init_jit(
    ...
    RepairProducerRuntime<false>::configure(
        custom_kernel_header, custom_source_sha);
    RepairProducerRuntime<true>::configure(
        custom_kernel_header, custom_source_sha);
}
...
inline void fp8_mqa_candidate_impl(
    ...
    if constexpr (kMaskedRepair) {
        DG_HOST_ASSERT(seq_len_kv > 0 and seq_len_kv <= kRepairCandidateCapacity);
    ...
    using Runtime = RepairProducerRuntime<kMaskedRepair>;
...
inline void fp8_mqa_candidate_repair_out(
    const torch::Tensor& q,
    const torch::Tensor& kv,
    const torch::Tensor& kv_scales,
    const torch::Tensor& weights,
