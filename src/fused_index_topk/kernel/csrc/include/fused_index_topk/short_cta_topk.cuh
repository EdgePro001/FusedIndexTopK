...
#ifndef ITK_SHORT_TUNING
#define ITK_SHORT_TUNING 0
#endif
#ifndef ITK_SHORT_DIAGNOSTIC
#define ITK_SHORT_DIAGNOSTIC 0
#endif
...
__device__ __forceinline__ uint64_t phase_clock() {
    if constexpr (ITK_SHORT_DIAGNOSTIC) return clock64();
    return 0;
}
...
// Short-context N=16384: uint16_t represents every valid index exactly. Scores
// remain full ordered FP32; this is a lossless layout change, not quantization.
template <>
struct CandidateStorage<5888> {
    ...
    __device__ __forceinline__ void store_pair(int row, int index, uint64_t pair) {
        ...
        indices[row][index] = static_cast<uint16_t>(pair);
    }
    __device__ __forceinline__ uint32_t load_score(int row, int index) const {
        return scores[row][index];
    }
    __device__ __forceinline__ uint64_t load_pair(int row, int index) const {
        return static_cast<uint64_t>(scores[row][index]) << 32 | indices[row][index];
    }
    __device__ __forceinline__ int32_t load_index(int row, int index) const {
        return indices[row][index];
    }
};
...
static_assert(sizeof(SelectionScratch<7680>) == 124064);
static_assert(sizeof(SelectionScratch<4096>) == 66720);
static_assert(sizeof(SelectionScratch<5888>) == 71840);
...
__device__ __forceinline__ void select_row(
    ...
    if (tid == 0) {
        ...
        *failure = scratch->failed;
        if constexpr (ITK_SHORT_DIAGNOSTIC) trace[12] = total;
    }
    selection_sync();
    if (scratch->failed) {
    ...
    for (int shift = 24; shift >= 0; shift -= 8) {
        ...
        if constexpr (ITK_SHORT_TUNING & 2) {
    ...
    for (int i = tid; i < kCandidateCapacity; i += 96) {
        ...
        uint64_t pair = 0;
        uint32_t score = 0;
        if constexpr (ITK_SHORT_TUNING & 1) {
