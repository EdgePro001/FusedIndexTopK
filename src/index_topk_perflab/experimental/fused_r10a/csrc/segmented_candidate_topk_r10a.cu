...
constexpr int kFastWorkingCapacity = 6656;
...
static_assert(kFastWorkingCapacity == 6656);
static_assert(kFastWorkingBytes == 53248);
...
void itk_fused_r10a_segmented_candidate_radix(
...
void configure() {
  cudaError_t status = cudaFuncSetAttribute(
      itk_fused_r10a_segmented_candidate_radix<
          kFastSegmentCapacity, false, kFastBlockThreads, 4>,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      kFastWorkingBytes);
  TORCH_CHECK(status == cudaSuccess,
              "cannot opt in to segmented R10a candidate shared memory: ",
  ...
  TORCH_CHECK(status == cudaSuccess,
              "cannot opt in to masked R10a repair shared memory: ",
              cudaGetErrorString(status));
}
...
pybind11::dict resource_report() {
  ...
  cudaError_t status = cudaFuncGetAttributes(
      ...
      itk_fused_r10a_segmented_candidate_radix<
          kFastSegmentCapacity, false, kFastBlockThreads, 4>);
  TORCH_CHECK(status == cudaSuccess,
              "cannot query segmented R10a candidate kernel: ",
              cudaGetErrorString(status));
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      kFastWorkingBytes);
  TORCH_CHECK(status == cudaSuccess,
              "cannot calculate segmented R10a candidate occupancy: ",
              cudaGetErrorString(status));
  ...
  status = cudaFuncGetAttributes(
      ...
      itk_fused_r10a_segmented_candidate_radix<
          kRepairSegmentCapacity, true, kRepairBlockThreads, 1>);
  TORCH_CHECK(status == cudaSuccess,
              "cannot query masked R10a repair kernel: ",
              cudaGetErrorString(status));
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      kRepairCandidateBytes);
  TORCH_CHECK(status == cudaSuccess,
              "cannot calculate masked R10a repair occupancy: ",
              cudaGetErrorString(status));
...
void topk_out_impl(torch::Tensor candidate_pairs,
  ...
  TORCH_CHECK(status == cudaSuccess,
              kMaskedRepair
                  ? "masked R10a repair radix launch failed: "
                  : "segmented R10a candidate radix launch failed: ",
              cudaGetErrorString(status));
}
