...
constexpr int kParallelSmemBytes = 225632;
constexpr int kCompactSmemBytes = 217920;
constexpr int kPackedSmemBytes = 228160;
...
public:
    ...
    static std::string generate_impl(const Args& args) {
        ...
        return fmt::format(R"(
#define ITK_LONG_TUNING {}
#define ITK_LONG_DIAGNOSTIC {}
#define ITK_LONG_MATH_SCHEDULE {}
#define ITK_LONG_MATH_REGISTERS {}
#define ITK_LONG_CACHE_WEIGHTS {}
#include "{}"
using namespace fused_index_topk::kernel::long_context;
static_assert(101632 + sizeof(SelectionScratch<7680>) == 225632);
static_assert(84608 + 2 * sizeof(SelectionScratch<4096>) == 217920);
...
static void __instantiate_kernel() {{
    ...
    static void launch_impl(const KernelHandle& kernel,
        ...
        TORCH_CHECK(initial_registers >= 64 and initial_registers <= args.math_registers,
                    "long-context fused TopK invalid initial register count for setmaxnreg inc/dec: ", initial_registers);
        TORCH_CHECK(256 * args.math_registers + 128 * 64 <= 384 * initial_registers,
                    "long-context fused TopK requested role registers exceed actual initial CTA pool: ",
        ...
        DG_CUDA_UNIFIED_CHECK(launch_kernel(
            ...
            args.start, args.end, args.thresholds, args.output, args.failures, args.trace, args.spill,
...
void fp8_mqa_topk_out(
    ...
    DG_HOST_ASSERT(spill.dim() == 3 and spill.size(0) == rows and
                   spill.size(1) == 8 and spill.size(2) == 256);
    // long-context fused TopK uses hierarchical 16K chunk repair; the fast kernel accepts up to 160K.
    DG_HOST_ASSERT(columns >= 8192 and columns <= 163840 and columns % 128 == 0 and heads == 64 and dim == 128 and kv_dim == dim);
    ...
    const Runtime::Args args = {
        ...
        .spill = reinterpret_cast<uint64_t*>(spill.data_ptr<int64_t>()),
    ...
    const auto runtime = compiler->build(
        "itk_fused_index_topk_long_" + std::to_string(mode) + "_" +
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.doc() = "Long-context GEMM + SMEM-resident exact TopK GEMM + SMEM-resident exact TopK";
    module.def("init_jit", &fused_index_topk::kernel::long_context::init_jit);
    module.def("fp8_mqa_topk_out", &fused_index_topk::kernel::long_context::fp8_mqa_topk_out);
}
