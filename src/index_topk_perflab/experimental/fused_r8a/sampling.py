"""R8a sampling utilities with a 128-thread threshold kernel."""
...
_SOURCE = Path(__file__).resolve().parent / "csrc" / "sampling_threshold_r8a.cu"
_EXTENSION: Any | None = None


def sample_elements_for_context(context_tokens: int) -> int:
    """Use R5i's unchanged 0.5% independent-token sampling schedule."""
...
def guarded_sample_rank(
    ...
    *,
    ...
    """Return R5i's unchanged conservative descending sample rank."""
...
def load_sampling_extension(*, verbose: bool = False) -> Any:
    ...
    if CUDA_HOME is None:
        raise RuntimeError("CUDA_HOME is required to build fused R8a sampling")
    identity = path_fingerprint(_SOURCE)
    module_name = f"itk_fused_r8a_sampling_{identity['sha256'][:12]}"
    _EXTENSION = load(
        name=module_name,
        sources=[str(_SOURCE)],
