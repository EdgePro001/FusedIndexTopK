"""Build and load the final DeepGEMM-style fused candidate producer."""
...
def load_candidate_producer(deep_gemm: Any, *, verbose: bool = False) -> Any:
    ...
    if CUDA_HOME is None:
        raise RuntimeError("CUDA_HOME is required to build the FusedIndexTopK producer")
    ...
    if not _KERNEL_HEADER.is_file():
        raise RuntimeError(f"missing FusedIndexTopK device source: {_KERNEL_HEADER}")
    ...
    if missing:
        raise RuntimeError(
            f"missing FusedIndexTopK build include directories: {missing}"
        )

    module_name = f"fused_index_topk_producer_{source_sha[:12]}"
    _EXTENSION = load(
        name=module_name,
        sources=[str(_CSRC / "fast_producer_bindings.cpp")],
